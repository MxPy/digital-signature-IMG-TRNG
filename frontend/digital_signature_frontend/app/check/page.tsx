...
export default function Home() {
  const [keys, setKeys] = useState({ signature: '', public_key: ''});
      ...
      <div className="hero min-h-screen" style={{backgroundImage: 'url(https://www.purepc.pl/image/news/2023/03/09_discord_testuje_mozliwosci_sztucznej_inteligencji_ai_odpowie_na_pytania_uzytkownikow_i_pomoze_w_moderacji_serwera_1_b.png)'}}>
        ...
        <div className="hero-content text-center">
          <div className="max-w-2xl text-wrap">
            {!fetched && !loading && <div className='flex flex-col items-center'><h1>Put Your file and publick key then Click "Check" to check signature of your file</h1><button className="btn btn-wide btn-primary" onClick={handleUploadFile}>Sign</button><label className="form-control w-full max-w-xs ">
            ...
            {fetched && !loading && <div>
                                      ...
                                      <div className="mockup-code">
                                        <pre><code>{keys.public_key}</code></pre>
                                      </div>
                                    </div>}
